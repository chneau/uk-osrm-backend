...
function get_direction_from_superrel(rel, relations)
    ...
    function set_result(direction, current_rel)
        if (result ~= nil) and (direction ~= nil) then
            print('WARNING: relation ' .. rel:id() .. ' is a part of more then one supperrelations ' .. result_id ..
                      ' and ' .. current_rel:id())
            result = nil
        else
            result = direction
...
function Relations.parse_route_relation(rel, way, relations)
    ...
    if t == 'route' then
        ...
        if role_direction ~= nil and tag_direction ~= nil and role_direction ~= tag_direction then
            ...
            print('WARNING: conflict direction in role of way ' .. way:id() .. ' and direction tag in relation ' ..
                      rel:id())
        end
        ...
        if (result_direction ~= nil) and (super_dir ~= nil) and (result_direction ~= super_dir) then
            print('ERROR: conflicting relation directions found for way ' .. way:id() .. ' relation direction is ' ..
                      result_direction .. ' superrelation direction is ' .. super_dir)
            result_direction = nil
