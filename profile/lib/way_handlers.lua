...
function WayHandlers.startpoint(profile, way, result, data)
    ...
    if profile.allowed_start_modes then
        ...
        result.is_startpoint = result.forward_mode == profile.default_mode or result.backward_mode ==
...
function WayHandlers.access(profile, way, result, data)
    data.forward_access, data.backward_access = Tags.get_forward_backward_by_set(way, data,
        profile.access_tags_hierarchy)
...
function WayHandlers.penalties(profile, way, result, data)
    ...
    local forward_penalty = math.min(highway_penalty, service_penalty, width_penalty, alternating_penalty,
        sideroad_penalty)
    local backward_penalty = math.min(highway_penalty, service_penalty, width_penalty, alternating_penalty,
        sideroad_penalty)
...
function WayHandlers.handle_length(profile, way, result, data)
    ...
    if forward and forward < profile.vehicle_length then
        if forward_conditional and string.match(forward_conditional, 'no(ne)? ?@') and
            (string.match(forward_conditional, 'destination') or string.match(forward_conditional, 'delivery')) then
            -- Discourage usage
    ...
    if backward and backward < profile.vehicle_length then
        if backward_conditional and string.match(backward_conditional, 'no(ne)? ?@') and
            (string.match(backward_conditional, 'destination') or string.match(backward_conditional, 'delivery')) then
            -- Discourage usage
...
function WayHandlers.handle_hgv_access(profile, way, result, data)
    ...
    if forward == 'no' and (not forward_conditional or
        not (string.match(forward_conditional, 'yes') or string.match(forward_conditional, 'destination') or
            string.match(forward_conditional, 'delivery'))) then
        -- No legal access at any condition, set a large weight
        result.forward_rate = math.min(result.forward_rate, (result.forward_speed * 0.1) / 3.6)
    end

    if backward == 'no' and (not backward_conditional or
        not (string.match(backward_conditional, 'yes') or string.match(backward_conditional, 'destination') or
            string.match(backward_conditional, 'delivery'))) then
        -- No legal access at any condition, set a large weight
...
function WayHandlers.oneway(profile, way, result, data)
    ...
    if profile.oneway_handling == true then
        oneway = Tags.get_value_by_prefixed_sequence(way, profile.restrictions, 'oneway') or
                     way:get_value_by_key("oneway")
    ...
    elseif profile.oneway_handling == true then
        local junction = way:get_value_by_key("junction")
        if data.highway == "motorway" or junction == "roundabout" or junction == "circular" then
            if oneway ~= "no" then
                -- implied oneway
                data.is_forward_oneway = true
                result.backward_mode = mode.inaccessible
